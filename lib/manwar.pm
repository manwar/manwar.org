...
get '/pullrequest/:year' => sub {

    my $year = params->{year};
    my $file = sprintf("pr-%d.json", $year);
    return send_data(path(setting('appdir'), 'public', 'stats', $file));
};

get '/pullrequest-challenge/:year' => sub {

    my $year = params->{year};
    my $file = sprintf("prc-%d.json", $year);
    return send_data(path(setting('appdir'), 'public', 'stats', $file));
};
...
get '/personal-distributions/:start/:end' => sub {
    ...
    my $end   = params->{end};
    my $file  = sprintf("personal-distributions-%d-%d.json", $start, $end);
    return send_data(path(setting('appdir'), 'public', 'stats', $file));
};
